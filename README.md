...
CodeNotes stores annotations externally (never in your repo) but anchors them using `git blame` — the same mechanism git uses to track where every line came from. When lines move during a rebase or merge, blame still knows their origin. Your notes follow automatically.

```
Your working tree          CodeNotes store (~/.codenotes/)
┌─────────────────┐        ┌──────────────────────────────┐
...
```bash
# VS Code
code --install-extension codenotes-0.2.0.vsix

# Cursor
cursor --install-extension codenotes-0.2.0.vsix
```

Or build from source:

```bash
git clone https://github.com/vaddisrinivas/codenotes.git
cd codenotes
npm install
npm run build
...
- **CodeNotes: Export Notes** — save all notes to a JSON file for backup
- **CodeNotes: Import Notes** — restore from a previous export (merges, deduplicates)
- **CodeNotes: Resolve Orphaned Notes** — re-anchor or delete notes whose lines were removed
- **CodeNotes: Refresh Notes** — force re-resolve all anchors (after a big rebase)
- **CodeNotes: Install Pre-Commit Hook** — safety net that blocks accidental note markers

## How It Works

### Blame-based anchoring

When you add a note on a line, CodeNotes runs `git blame` to find that line's **origin**: the commit, file, and line number where it was first introduced. This triple `(commit, file, line)` is a stable identity — it doesn't change when the line moves around.

On every file open/save, CodeNotes re-runs blame and matches stored anchors to current line positions:
...
Notes live at `~/.codenotes/<repo-hash>/notes.json`. Completely outside your repo — nothing to `.gitignore`, zero risk of committing.
...
|---------|---------|-------------|
| `codenotes.inlinePreview` | `true` | Show inline text after annotated lines |
| `codenotes.maxInlineLength` | `60` | Max characters in inline preview |
| `codenotes.orphanWarning` | `true` | Show warning decorations for orphaned notes |
